...
- rev2.5.0: 版本字串同步（HackMD 插件 rev2.5.0 上線）
- rev2.4.5: 版本字串同步（CWA/TDX SSL 降級 + Authorization 金鑰過濾）
- rev2.4.4: 版本字串同步（歷史對話掛 tools + tool loop 完整 log）
...
def home():
    """首頁"""
    return 'Hello, World! LINEBOT rev2.6.0 is running.'


@app.route('/about')
def about():
    """關於頁面（也用於 keepalive ping）"""
    return '<h1>LINEBOT rev2.6.0 - Python Flask LINE Bot (google-genai SDK + SQLite + Reminder)</h1>'
