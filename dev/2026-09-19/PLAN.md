...
## 決策定案（2026-09-19 討論結果，2026-09-23 增補 D6/D7）
...
| D6 | SPEC 條文解耦 | **SPEC 只記機制與政策**（載入白名單、重複 name raise、REQUIRED_ENV、三上限、per-tool 風險分級、async 支援），不記工具資產清單；現有 weather 庫存條文一併改寫為機制化描述。HackMD 專屬設定（token、白名單、群組語義）留 plugin 層與 README。**新增 plugin 不再觸動 SPEC** |
| D7 | 身份標的語義 | 個人身份（user）：WRITE/DESTRUCTIVE 需在 `HACKMD_ALLOWED_USER_IDS` 白名單；**群組/聊天室身份（group/room）一律唯讀**——list/read/search 可用，create/update/delete 全禁（即使 group_id 在白名單）。`line_handler._get_user_id()` 依來源回傳 user/group/room ID，此語義列為機制+部署文件層決策，SPEC 只記機制條文 |
...
| 6 | SPEC 機制化 + 文件同步（D6/D7） | `SPEC.md`、`README.md`、`CHANGELOG.md`、`MEMOIR.md` | P1 |
...
- 在 `ai_text.py` 的執行前檢查 `policy`：若 handler policy 非 READ_ONLY（WRITE/DESTRUCTIVE），先看身份型別——group/room 一律拒絕；僅 user 身份比對 `HACKMD_ALLOWED_USER_IDS`；不在 → 回傳 `{"error": "此操作未經授權"}`，**不執行 handler**
- 身份傳遞路徑：`line_handler._handle_text_message` → `chat_with_ai` → `AITextService.chat` → `_auto_handle_tool_calls`（新增 `user_id` + `user_scope` 參數或 context）
...
### 階段 6：SPEC 機制化與文件同步（D6/D7，2026-09-23 增訂）

- 改寫 `SPEC.md` plugin 條文為純機制描述：抽離 weather 工具清單，加入 per-tool risk 分級（READ_ONLY/WRITE/DESTRUCTIVE）執行前檢查機制
- 工具資產清單（hackmd 6 支、weather 4 支）移往 `README.md` 啟用目錄；plugin 模組自帶 REQUIRED_ENV/TOOLS/POLICY
- 於 README/plugin 文件註記 `_get_user_id()` 的 user/group/room 語義與 `HACKMD_ALLOWED_USER_IDS` 填寫規則（群組以 group_id 為單一身份且**一律唯讀**，D7）
- 版本戳記：`SPEC.md` 的 `document_version` 遞增；`project_version` 留待 release 時改 2.5.0
- **目的：日後新增 plugin（如 tra）不再觸動 SPEC，只動 plugin 模組 + README + CHANGELOG + MEMOIR**
